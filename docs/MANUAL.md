...
| [preview.py](../kineticpulse/vision/preview.py) | Live annotated preview for the running orchestrator (`--preview`): posture bbox, COCO-17 skeleton, a MOTION panel (vision features + accelerometer magnitude and signature), a FUSION panel (tier, signatures, BPM, reason) and an amber provenance band when the telemetry is synthetic. Renders to a window, to a file (`--preview-snapshot`, for SSH sessions), or both. Every failure path degrades to "no preview" rather than taking the pipeline down. |
| [control.py](../kineticpulse/control.py) | Runtime scenario control behind `monitoring.control_enabled`. Gates and audits the `POST /control/*` endpoints the dashboard's `/control` panel drives, and builds the `simulation` block that marks a drill on the monitoring and alert payloads. **Bench only** — a Tier-2 scenario runs the real dispatch path. |
| [ppg_sim.py](../kineticpulse/sensors/ppg_sim.py) | Synthetic resting PPG for a dead/absent MAX30102 (`wristband.ppg_source: simulated`). Motion still comes from the hardware transport; the heart rate is labelled as simulated in the logs, the monitoring payload, the dashboard, the alert payload and the event history. **Bench only** — pulse loss and the cardiac tier cannot fire in this mode. |
...
| [tests/test_ppg_sim.py](../tests/test_ppg_sim.py) | Synthetic PPG waveform quality (50-120 BPM through the real estimator), clock-independent burst pacing, and the simulated-HR labelling on every surface | 24 |
| [dashboard/tests/liveRender.test.tsx](../dashboard/tests/liveRender.test.tsx) | Replays payloads captured off a running Jetson through the real dashboard components, so backend field renames break a test instead of silently blanking a banner | 2 |
| [tests/test_export.py](../tests/test_export.py) | Export format parsing and the INT8 calibration contract — `--int8` must refuse to run without `--data`, since Ultralytics otherwise calibrates on downloaded COCO8 images | 7 |
| [tests/test_preview.py](../tests/test_preview.py) | Preview frame timing, overlay drawing via the snapshot output, edge-clamped detection labels, and every degradation path (no display, no GUI, bad snapshot path, GUI dying mid-run) | 21 |
| [tests/test_control.py](../tests/test_control.py) | `ScenarioClock` timing, runtime scenario switching in the mock client, control gating (disabled / unavailable / unknown), the `/control` endpoints, and the drill markers on both payloads | 30 |
